...
public:
    ...
    bool operator==(const Rota& source) const { return eq(source); }
    ...
    /// \brief Get the event IDs for this Rota
    /// \return the list of IDs of events for the Rota
    std::vector<int>& get_events() { return events; }

    /// \brief Set the event IDs for the Rota
    /// \param[in] e The new list of event IDs
    ///
    /// Calling this method **replaces** the existing list of events.
    void set_events(const std::vector<int>& e) { events = e; }
    /// @}
    ...
    /// \brief Add events to the rota
    /// \param[in] to_add Events to be added
    ///
    /// Merges the new events with the existing events list. Also sorts events
    /// by ID.
    void add_events(const std::vector<int>& to_add);

    /// \brief Remove events from the rota
    /// \param[in] to_remove Events to be remove
    void remove_events(const std::vector<int>& to_remove);
...
protected:
    ...
    /// included.
    virtual bool eq(const Rota& rhs) const;
