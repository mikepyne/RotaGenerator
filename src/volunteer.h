...
public:
    ...
    Volunteer(const nlohmann::json& v);
    ...
    Volunteer& operator=(const nlohmann::json& v);
    ...
    bool operator==(const Volunteer& source) const;
    ...
    void set_first_name(const std::string& fn) { firstName = fn; }
    ...
    void set_last_name(const std::string& sn) { lastName = sn; }
    ...
    void set_phone_home(const std::string& ph) { phoneHome = ph; }
    ...
    void set_phone_mobile(const std::string& pm) { phoneMobile = pm; }
    ...
    const std::string get_name() const { return firstName + " " + lastName; };

    /// \brief Set combined first and last name
    /// \param[in] name The combined name
    ///
    /// Splits the name on the first space; everything up to the first space is
    /// first name and everything after the first space is last name.
    /// If there is no space, everything is put in last name.
    /// Perhaps a naive approach, so we'll see what happens when this is
    /// actually used for real.
    void set_name(const std::string& name);

    /// \brief Nlohmann JSON boilerplate
    NLOHMANN_DEFINE_TYPE_INTRUSIVE(Volunteer,
                                   id,
                                   firstName,
                                   lastName,
                                   phoneHome,
...
protected:
    ...
    /// mocked.
    virtual bool eq(const Volunteer& rhs) const;
